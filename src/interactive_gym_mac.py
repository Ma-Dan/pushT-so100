...
# 运行方式: mjpython interactive_gym_mac.py --policy <checkpoint_path> --dataset <dataset_path>
# ============================================================================
import os
# 使用 glfw 渲染
...
class DiffusionPolicyWrapper:
    """Diffusion Policy 包装器，用于与 PushT 环境交互"""

    def __init__(self, policy, preprocessor, postprocessor, dataset_metadata, device):
        """
        Args:
            policy: DiffusionPolicy 模型
            preprocessor: 预处理器
            postprocessor: 后处理器
...
def load_diffusion_policy(policy_path, dataset_path, device):
    """
    加载 Diffusion Policy 模型
    ...
    Returns:
        policy_wrapper: DiffusionPolicyWrapper 实例
    """
    ...
    # 加载模型
    policy = DiffusionPolicy.from_pretrained(policy_path.absolute())
    policy.eval()
    policy.to(device)

    # 加载数据集元数据
    dataset_metadata = LeRobotDatasetMetadata(dataset_path.absolute())
...
def make_env_and_policy(xml_path=None, policy_path=None, dataset_path=None):
    """
    初始化本地 SO-ARM100 PushT 环境和策略
    ...
    if policy_path is not None:
        if dataset_path is None:
            ...
            try:
                policy = load_diffusion_policy(policy_path, dataset_path, device)
...
if __name__ == "__main__":
    ...
    #
    #   mjpython interactive_gym_mac.py
    #
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Interactive SO-ARM100 PushT Environment')
    parser.add_argument('--xml', type=str, default=None,
                        help='Path to MuJoCo XML file (default: human_env.xml)')
    parser.add_argument('--policy', type=str, default=None,
                        help='Path to policy checkpoint directory (e.g., outputs/ckpt/final_model)')
    parser.add_argument('--dataset', type=str, default=None,
                        help='Path to dataset directory (required when loading policy, e.g., data/NewData3.9-ee-2d-pos)')
