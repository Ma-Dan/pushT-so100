...
# 运行方式: mjpython interactive_gym_act.py --policy <checkpoint_path> --dataset <dataset_path>
# ============================================================================
import os
# 使用 glfw 渲染
...
class ACTPolicyWrapper:
    ...
    def __call__(self, observation):
        ...
        with torch.no_grad():
            ...
            # 对于 ACT，需要处理观测特征的时间维度
            # ACT 期望的观测数据形状为 (B, feature_dim)，而不是 (B, 1, feature_dim)
...
def load_act_policy(policy_path, dataset_path, device):
    """
    加载 ACT Policy 模型
    ...
    Returns:
        policy_wrapper: ACTPolicyWrapper 实例
    """
    ...
    # 加载模型
    policy = ACTPolicy.from_pretrained(policy_path.absolute())
    policy.eval()
    policy.to(device)
...
def make_env_and_policy(xml_path=None, policy_path=None, dataset_path=None):
    """
    初始化本地 SO-ARM100 PushT 环境和 ACT 策略
    ...
    if policy_path is not None:
        if dataset_path is None:
            ...
            try:
                policy = load_act_policy(policy_path, dataset_path, device)
...
def main(env, policy):
    ...
    运行主要的交互仿真循环
    """
    print("Starting interactive simulation with ACT policy...")
...
if __name__ == "__main__":
    ...
    #
    #   mjpython interactive_gym_act.py
    #
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Interactive SO-ARM100 PushT Environment with ACT Policy')
    parser.add_argument('--xml', type=str, default=None,
                        help='Path to MuJoCo XML file (default: human_env.xml)')
    parser.add_argument('--policy', type=str, default=None,
                        help='Path to ACT policy checkpoint directory (e.g., outputs/pusht_act/final_model)')
